export const cards = [
  {
    id: "1",
    title: "Ecole de musique de Toulouse : ",
    description:
      "Pour prendre des cours d'instruments, de pratique collective ou de théorie musicale, nous vous acceuillons à bras ouverts.",
    imageUrl: "/images/ecole.jpg",
    mail: "assoinextremis@gmail.com",
  },
  {
    id: "2",
    title: "Atelier théorique à prix libre en ligne :",
    description:
      "Notre serveur discord vous acceuille pour des ateliers théoriques en ligne et en groupe sur des sujets comprends que vous pouvez proposer, le tout à prix libre et conscient.",
    imageUrl: "/images/discord.jpg",
    link: "https://discord.gg/8Z8Q6Z8",
  },
  {
    id: "3",
    title: "Nos Stages : ",
    description:
      "Retrouvez nos stages de musique en groupe, pour tous les niveaux et tous les âges sur notre site : ",
    link: "https://stageMusicaldulot.fr",
    imageUrl: "/images/stage.jpg",
  },
];
