...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('Cookies extraits - csrftoken:', csrftoken, 'sessionid:', sessionid);
    ...
    if (!token) {
      ...
      return NextResponse.json(
        { error: 'Authentication required', details: 'No token found in headers or cookies' },
        { status: 401 }
      );
    }
    ...
    console.log('TOKEN REÇU DU FRONTEND (uploadToken):', uploadTokenFromFrontend);
    ...
    if (!uploadTokenFromFrontend) {
      try {
        console.log('Tentative de récupération d\'un token spécifique d\'upload...');
        ...
        if (tokenResponse.ok) {
          ...
          if (Array.isArray(tokensData) && tokensData.length > 0) {
            ...
            console.error('Format de réponse inattendu ou liste de tokens vide');
          }
        } else {
          const errorText = await tokenResponse.text();
          console.error(`Erreur lors de la récupération du token d'upload: ${tokenResponse.status} - ${errorText}`);
        }
      } catch (error) {
        console.error('Exception lors de la récupération du token d\'upload:', error);
      }
    }
    ...
    console.log('Headers:', Object.fromEntries(mediaCmsResponse.headers.entries()));
    ...
    if (!mediaCmsResponse.ok) {
      console.error(`Erreur MediaCMS: ${mediaCmsResponse.status} - ${responseText}`);
    ...
    const mediaCmsId = mediaCmsData.friendly_token || mediaCmsData.url?.split('/').pop();
    const videoUrl = mediaCmsData.url || `http://localhost:9000/api/v1/media/${mediaCmsId}/`;
    const thumbnailUrl = mediaCmsData.thumbnail_url || `http://localhost:9000/api/v1/media/${mediaCmsId}/thumbnail/`;
    ...
    const graphqlEndpoint = process.env.NEXT_PUBLIC_GRAPHQL_ENDPOINT || 'http://localhost:8000/graphql';
    ...
    return NextResponse.json(
      { error: `Upload processing error: ${error instanceof Error ? error.message : String(error)}` },
      { status: 500 }
    );
  }
}
