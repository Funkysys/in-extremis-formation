...
export default function UserInfosForm({ onDeleted, onLoginRequest }: UserInfosFormProps) {
  ...
  if (loading) return (
    <div className="container mx-auto py-8 px-4 min-h-screen max-w-lg">
      <h1 className="text-3xl font-bold mb-6 text-slate-100 font-roboto border-b border-slate-200">Chargement...</h1>
    </div>
  );
  ...
  if (!data?.me) return (
    <div className="container mx-auto py-8 px-4 min-h-screen max-w-lg">
      <h1 className="text-3xl font-bold mb-6 text-slate-100 font-roboto border-b border-slate-200">Aucun utilisateur connecté</h1>
    </div>
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.phone && !isValidPhone(form.phone)) {
      setPhoneError("Numéro de téléphone invalide (format international attendu).");
      return;
    }
    try {
      await updateUser({
  ...
  const handleDelete = async () => {
    ...
    if (!data?.me?.id) {
      ...
      addToast("Impossible de supprimer le compte : identifiant utilisateur introuvable.", "error");
      return;
    }
    try {
  ...
  return (
    <div className="container mx-auto py-8 px-4 min-h-screen max-w-lg">
      <h1 className="text-3xl font-bold mb-6 text-slate-100 font-roboto border-b border-slate-200">Mes informations</h1>
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block mb-1 font-semibold">Nom complet <span className="text-red-500">*</span></label>
          <input
            type="text"
            name="fullName"
            value={form.fullName}
