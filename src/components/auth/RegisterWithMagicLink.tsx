...
export default function RegisterWithMagicLink({ onSuccess }: { onSuccess: () => void }) {
  ...
  const [requestMagicLink, { loading }] = useMutation(REQUEST_MAGIC_LINK_MUTATION, {
    onCompleted: (data) => {
      if (data.requestMagicLink && data.requestMagicLink.success) {
        setMessage("Lien magique envoyé ! Consulte ta boîte mail pour finaliser l'inscription.");
      } else {
  ...
  const [verifyMagicLink, { loading: loadingVerify }] = useMutation(VERIFY_MAGIC_LINK_MUTATION, {
    onCompleted: (data) => {
      if (data.verifyMagicLink?.token) {
        localStorage.setItem("token", data.verifyMagicLink.token);
        onSuccess();
      } else {
        setError(data.verifyMagicLink?.error || "Lien invalide.");
      }
    },
    onError: (err) => setError(err.message),
  });
    ...
    <div className="flex flex-col gap-4 w-full max-w-xs mx-auto">
      ...
      <form onSubmit={handleVerify} className="form-control gap-2">
        ...
        <button className="btn btn-accent mt-2" disabled={loadingVerify} type="submit">
          {loadingVerify ? "Vérification..." : "S'inscrire avec le token"}
        </button>
      </form>
