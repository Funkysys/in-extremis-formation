"use client";
import {
  CREATE_VIDEO_MARKER_MUTATION,
  DELETE_VIDEO_MARKER_MUTATION,
...
import { Video, VideoMarker } from "@/types/video";
...
export default function VideoChaptersModal({
  video,
  isOpen,
  onClose,
}: VideoChaptersModalProps) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const [chapterTitle, setChapterTitle] = useState("");
  const [chapterDesc, setChapterDesc] = useState("");
  const [timestamp, setTimestamp] = useState<number>(0);
  const { data, loading, error, refetch } = useQuery(
    VIDEO_MARKERS_BY_VIDEO_QUERY,
    {
  ...
  const handleAddChapter = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveError("Erreur lors de l'ajout du chapitre");
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-2 sm:p-6 md:p-8 max-w-full sm:max-w-2xl w-full relative">
        ...
        <h2 className="text-xl font-bold mb-4">
          Chapitrer la vidéo : {video.title}
        </h2>
        <video
          ref={videoRef}
          src={video.url}
        ...
        {loading ? (
          ...
          <ul className="space-y-2">
            {data?.videoMarkers?.map(
              (m: VideoMarker, idx: number, arr: VideoMarker[]) => (
                <li
                  key={m.id}
                ...
                >
                  ...
                  <span className="text-xs text-slate-500">
                    {Math.floor(m.timestamp / 60)}:
                    {(Math.floor(m.timestamp) % 60).toString().padStart(2, "0")}
                  </span>
                  <span className="text-xs sm:text-sm">{m.description}</span>
                  <button
                    className="btn btn-xs btn-error ml-0 sm:ml-2 mt-1 sm:mt-0"
                    onClick={async () => {
                      if (
                        !window.confirm(
                          "Confirmer la suppression de ce chapitre ?"
                        )
                      )
                        return;
                      try {
