...
export function SecureVideoPlayer({ videoId, title, thumbnailUrl, className = '' }: SecureVideoPlayerProps) {
  const { user } = useAuth()
  const [videoToken, setVideoToken] = useState<string | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [isLoading, setIsLoading] = useState(true)

  const { data, loading, error: tokenError } = useQuery(GENERATE_VIDEO_TOKEN, {
    variables: { videoId },
    skip: !user || !videoId,
  ...
  useEffect(() => {
    if (data?.generateVideoToken?.success) {
      ...
      setError(data.generateVideoToken.message || 'Erreur lors de la génération du token')
      setIsLoading(false)
    }
  }, [data])
  ...
  if (isLoading) {
    return (
      <div className={`aspect-video bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center ${className}`}>
        <div className="animate-pulse text-gray-500">Chargement de la vidéo...</div>
      </div>
    )
  }
