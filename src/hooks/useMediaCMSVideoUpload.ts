...
export const useMediaCMSVideoUpload = (
  options: UseMediaCMSVideoUploadOptions = {}
) => {
    ...
    async (file: File, title: string) => {
      ...
      if (file.size > maxSize) {
        const error = new Error(
          `Le fichier est trop volumineux (max ${maxSizeMB}MB)`
        );
        showToast(error.message, "error");
        onError?.(error);
        throw error;
...
export type MediaCMSVideoUploadProps = ReturnType<
  typeof useMediaCMSVideoUpload
>;
