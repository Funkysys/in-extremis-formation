export async function uploadVideoMultipart({
  ...
  const restEndpoint = endpoint.replace('/graphql', '/api/uploads/upload-video');
  ...
  try {
    ...
    throw new Error("La réponse du serveur n'est pas au format JSON. Réponse brute : " + responseText);
  }
  return result;
}
